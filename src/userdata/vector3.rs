...
use mlua::prelude::*;
use crate::{core::LuaSingleton, enums::{Axis, NormalId}, internal_prelude::*};
...
impl LuaUserData for Vector3 {
    fn add_fields<F: LuaUserDataFields<Self>>(fields: &mut F) {
        fields.add_field_method_get("X", |_lua, this| {
            Ok(this.0.x)
        });
        fields.add_field_method_get("Y", |_lua, this| {
            Ok(this.0.y)
        });
        fields.add_field_method_get("Z", |_lua, this| {
            Ok(this.0.z)
        });
        fields.add_field_method_get("Magnitude", |_lua, this| {
            Ok(this.0.length())
        });
        fields.add_field_method_get("Unit", |_lua, this| {
    ...
    fn add_methods<M: LuaUserDataMethods<Self>>(methods: &mut M) {
        methods.add_method("Abs", |_lua, this, ()| {
            Ok(Self(this.0.abs()))
        });
        methods.add_method("Ceil", |_lua, this, ()| {
            Ok(Self(this.0.ceil()))
        });
        methods.add_method("Floor", |_lua, this, ()| {
            Ok(Self(this.0.floor()))
        });
        methods.add_method("Sign", |_lua, this, ()| {
            Ok(Self(this.0.signum()))
        });
        ...
        methods.add_method("Dot", |_lua, this, (o,): (Vector3,)| {
            Ok(this.0.dot(o.0))
        });
...
impl LuaSingleton for Vector3 {
    fn register_singleton(lua: &Lua) -> LuaResult<()> {
        ...
        table.raw_set("FromNormalid", lua.create_function(Vector3::from_normal_id)?)?;
